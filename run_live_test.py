#!/usr/bin/env python3
"""
Conservative Live Test Runner for Cross-Exchange Arbitrage Bot

This script runs a 2-3 hour live test with strict risk controls:
...
class LiveTestRunner:
    """Manages the live test execution."""
    ...
    def _signal_handler(self, signum, frame):
        ...
        logger.info(f"Received signal {signum}, stopping live test...")
        self.running = False
    
    async def run_test(self):
        """Run the live test."""
        try:
            logger.info("🚀 Starting Conservative Live Test")
            logger.info("=" * 60)
            ...
            if config.exchanges.accounts["binance"].sandbox:
                logger.error("❌ Binance is in sandbox mode! Set sandbox: false for live trading")
                return
            
            if config.exchanges.accounts["okx"].sandbox:
                logger.error("❌ OKX is in sandbox mode! Set sandbox: false for live trading")
                return
    ...
    async def _monitor_session(self):
        """Monitor the live test session."""
        logger.info("📊 Monitoring live test session...")
