...
from ..core.detector import ArbitrageOpportunity, ArbitrageDirection
...
class ArbitrageExecutor:
    ...
    async def _execute_atomic_hedge(self, opportunity: ArbitrageOpportunity, start_time: float) -> ExecutionResult:
        ...
        left_order, right_order = await asyncio.gather(buy_task, sell_task, return_exceptions=True)
        
        # Check for exceptions
        if isinstance(left_order, Exception):
            left_order = OrderResult(False, error=str(left_order))
        if isinstance(right_order, Exception):
            right_order = OrderResult(False, error=str(right_order))
        
        # Check execution results
        if not left_order.success or not right_order.success:
            # Cancel successful order if other failed
        ...
        if left_fill_ratio < 0.95 or right_fill_ratio < 0.95:
            # Partial fills - cancel remaining
            if self.cancel_on_partial:
